...
const Page = () => {
  ...
  const wordColors = [
    { text: 'AI', color: 'text-primary-500' },
    { text: 'Magic', color: 'text-blue-600' },
    { text: 'Ease', color: 'text-green-600' }
  ];

  useEffect(() => {
    const interval = setInterval(() => {
      const currentIndex = wordColors.findIndex(item => item.text === currentWord);
      const nextIndex = (currentIndex + 1) % wordColors.length;
      setCurrentWord(wordColors[nextIndex].text);
    }, 2000); // Change word every 2 seconds

    return () => clearInterval(interval);
  }, [currentWord]);

  const currentColor = wordColors.find(item => item.text === currentWord)?.color || 'text-primary-500';
    ...
    <div>
      ...
      <section>
        <div className="py-8 px-6 mx-auto max-w-screen-xl text-center lg:py-16 lg:px-12 md:px-10">
          <h1 className="mt-4 lg:mt-8 mb-4 text-4xl font-extrabold tracking-tight leading-none text-black md:text-5xl lg:text-6xl">
            Build Your Resume <span className={`${currentColor} max-sm:block transition-all duration-500 ease-in-out`}>
              With {currentWord}
            </span>
          </h1>
      ...
      <footer className="backdrop-blur-md w-full">
        <div className="w-full mx-auto text-center max-w-screen-xl p-4 flex max-md:flex-col md:items-center md:justify-between">
          ...
          <Link href="https://github.com/Majormiles" className="me-4 md:me-6">
